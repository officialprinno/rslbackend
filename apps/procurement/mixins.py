"""Shared ViewSet behaviour for procurement API endpoints."""
...
class ProcurementViewSetMixin:
    """Standard FMS response envelope and RBAC for procurement viewsets."""

    module_name = "procurement"
    filter_backends = [FMSDjangoFilterBackend, FMSSearchFilter, FMSOrderingFilter]

    def get_required_action(self):
        if self.action in ("approve", "reject", "match", "pay"):
            return "approve"
        if self.action in (
            "create",
            "submit",
