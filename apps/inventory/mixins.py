"""Shared ViewSet behaviour for inventory API endpoints."""
...
class InventoryViewSetMixin:
    """Standard FMS response envelope and RBAC for inventory viewsets."""

    module_name = "inventory"
    filter_backends = [FMSDjangoFilterBackend, FMSSearchFilter, FMSOrderingFilter]

    def get_required_action(self):
        if self.action in ("create", "approve", "reject", "mark_read", "complete", "issue"):
            return "create" if self.action == "create" else "approve"
    ...
    def get_permissions(self):
        ...
        if self.action in ("approve", "reject", "complete", "issue"):
