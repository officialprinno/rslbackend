"""Shared ViewSet behaviour for finance API endpoints."""
...
class FinanceViewSetMixin:
    """Standard FMS response envelope and RBAC for finance viewsets."""

    module_name = "finance"
    filter_backends = [FMSDjangoFilterBackend, FMSSearchFilter, FMSOrderingFilter]

    def get_required_action(self):
        if self.action in ("post_entry", "complete", "reverse", "make_payment"):
            return "approve"
